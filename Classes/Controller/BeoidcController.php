...
class BeoidcController extends ActionController
{
    ...
    public function requestAction(): ResponseInterface
    {
        ...
        $this->pageRenderer->addCssFile('EXT:oauth/Resources/Public/Css/oauth/main.css');
        $this->pageRenderer->loadJavaScriptModule('EXT:oauth/Resources/Public/JavaScript/oauth/main.js');
        ...
        if ($send_email == NULL) {
            ...
            if ($uid == null) {
                $encryptedCount = \Miniorange\Oauth\Helper\EncryptionHelper::encryptUserCount(10);
                if($typo3Version > 12){
                $queryBuilder->insert(Constants::TABLE_OIDC)->values(['uid' => 1,Constants::COUNTUSER => $encryptedCount,Constants::EMAIL_SENT => 1])->executeStatement();
                }else
                {
                    $queryBuilder
                    ...
                    ->values([
                        'uid' => 1,
                        Constants::COUNTUSER => $encryptedCount,
...
//------------ OPENID CONNECT SETTINGS---------------
        if (isset($_POST['option']) and $_POST['option'] == "oidc_settings") {
            if (!empty($_POST['redirect_url']) && !empty($_POST['feoidc']) && !empty($_POST['app_name']) && !empty($_POST['client_id']) && !empty($_POST['client_secret']) && !empty($_POST['scope']) && !empty($_POST['auth_endpoint']) && !empty($_POST['token_endpoint'])) {
                // Handle credentials location checkboxes (allow both to be selected)
                // When checkbox is checked, it sends 'on', when unchecked, the hidden field sends 'off'
                $headerChecked = isset($_POST['set_header_credentials']) && $_POST['set_header_credentials'] == 'on';
                $bodyChecked = isset($_POST['set_body_credentials']) && $_POST['set_body_credentials'] == 'on';
                
                // Explicitly set the values for both checkboxes
                $_POST['set_header_credentials'] = $headerChecked ? 'true' : 'false';
                $_POST['set_body_credentials'] = $bodyChecked ? 'true' : 'false';
                
                // Ensure at least one is selected (default to header if neither is selected)
                if (!$headerChecked && !$bodyChecked) {
                    $_POST['set_header_credentials'] = 'true';
                }
...
// --------------------SUPPORT QUERY---------------------
    ...
    {
        $count = 10;
        \Miniorange\Oauth\Helper\EncryptionHelper::updateEncryptedUserCount($count);
    }
}
